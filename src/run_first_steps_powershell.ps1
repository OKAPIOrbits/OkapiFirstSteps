...
wget -Uri http://okapi.ddns.net:34569/predict-passes/sgp4/requests -Headers @{"content-type"="application/json";"Accept"="application/json";"access_token"=$token} -Method POST -Body $(get-content pass_prediction_request.json -raw) -OutFile "request_response.json" 
...
wget -Uri http://okapi.ddns.net:34569/predict-passes/sgp4/summary/results/${request_id} -Headers @{"access_token"=$token} -OutFile "pass_prediction_result.json"
